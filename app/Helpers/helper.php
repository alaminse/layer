 ...
 */
    ...
    function getDefaultCurrency($prop = null)
    {
        ...
        $default_currency_key = array_search(config('configs.currency'), array_column(getVar('currency'), 'name'));
 ...
 */
    function currency($amount, $symbol = 0)
    {
        ...
        return $currency_symbol . ' ' . number_format($amount, $decimal_value);
...
if (!function_exists('single_price')) {
    function single_price($price)
    {
        ...
        if (config('configs.currency_symbol')) {
            return config('configs.currency_symbol') . " " . number_format($price, 2);
        } else {
...
if (!function_exists('rtl')) {
    function rtl()
    {
        ...
        if (\Illuminate\Support\Facades\Auth::check() and auth()->user()->language) {
            ...
            return (bool)$language ? $language->rtl : $default;
        } else {
            return $default;
        }
    }
...
function case_file($file)
{
    ...
    if (check_https() == 'https') {
        $file_path = str_replace('http:', 'https:', $file->filename);
    } else {
        $file_path = str_replace('https:', 'http:', $file->filename);
    }

    if (in_array($file->file_type, $image)) {
        return '<img src="' . $file_path . '" alt="' . $file->user_filename . '" width="100%" height="100%" />';
    }

    return '<embed src="' . $file_path . '" alt="' . $file->user_filename . '" type="' . $file->file_type . '" alt="' . $file->user_filename . '" width="100%" height="450px" />';
}
...
if (!function_exists('insertNewPermissions')) {
    function insertNewPermissions($sql)
    {
        $ids = collect($sql)->pluck('id')->toArray();
...
if (!function_exists('getOrganization')) {
    function getOrganization()
    {
        ...
        if (app()->bound('organization') && app('organization')) {
        ...
        if (file_exists($saas_module)) {
            ...
            if (isset($module_status['AdvSaas']) && $module_status['AdvSaas']) {
                ...
                if ($domain) {
                    $organization = Organization::where(['subdomain' => $domain, 'status' => 1])->firstOrFail();
                    if ($request->route()) {
                    ...
                    $organization = Organization::where(['subdomain' => $host, 'status' => 1])->firstOrFail();
                } elseif (auth()->check()) {
